"""
Architecture 2 -- Phase 2: Bronze → Silver

Reads the Bronze Delta table (populated by the Structured Streaming job),
applies type casts, validates data quality constraints, deduplicates orders
(latest record per order_id), and upserts the result into the Silver Delta
table using a MERGE operation.

This job is intentionally batch-oriented (run on a schedule or ad-hoc).
It shares the same transformation logic as arch1's bronze_to_silver.py,
with paths adjusted for Architecture 2.

Usage (inside the Spark container):
    spark-submit --master spark://spark-master:7077 /jobs/arch2/bronze_to_silver.py
"""
...
# Configuration
# ---------------------------------------------------------------------------
BRONZE_PATH = "/data/arch2/bronze/orders"
SILVER_PATH = "/data/arch2/silver/orders"
...
def create_spark_session() -> SparkSession:
    ...
    return (
        ...
        .appName("arch2-bronze-to-silver")
...
def deduplicate(df: DataFrame) -> DataFrame:
    ...
    order_timestamp using a Window rank function.
    """
...
def select_silver_columns(df: DataFrame) -> DataFrame:
    """Select and order the columns for the Silver layer.

    Arch2 streaming metadata (_kafka_partition, _kafka_offset, _kafka_timestamp)
    replaces arch1's _source_file / _batch_id columns.
    """
    # Dynamically include whatever metadata columns are present
    base_cols = [
        "order_id",
        "customer_id",
        "customer_name",
    ...
    # Add streaming-specific metadata if present
    optional_cols = ["_kafka_partition", "_kafka_offset", "_kafka_timestamp"]
    existing_optional = [c for c in optional_cols if c in df.columns]

    return df.select(*(base_cols + existing_optional))
...
def main():
    logger.info("=== Bronze to Silver (Architecture 2) ===")
